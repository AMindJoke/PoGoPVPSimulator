...
const plannerVersion = require("../src/reliability/battle-reliability").BATTLE_ENGINE_VERSION.replace("battle-", "");
assert.match(html, new RegExp(`src/battle/battle-intelligence\\.js\\?v=\\d+-${plannerVersion}"`), "The page must load the current battle planner version instead of reusing a stale browser-cached script.");
...
assert.match(html, /const technicalFork = manualComparisonTechnicalFork\(comparison, viewModel\);[\s\S]{0,100}const divergence = technicalFork \|\| semanticDivergence/, "Technical issue targets must take priority over generic semantic divergence.");
...
assert.match(html, /\["fast", "charge", "shield", "form-protect", "technical-lag", "technical-dre", "technical-anomaly"\]/, "Manual timeline hover details must cover normal and technical battle events.");
...
assert.match(html, /id="manualBringNextSearch" type="search"/);
assert.match(html, /id="manualBringNextCandidates" class="manual-bring-next-candidates" role="listbox"/);
assert.match(html, /function renderManualBringNextCandidates/);
assert.match(html, /data-manual-bring-next-pokemon/);
assert.doesNotMatch(html, /id="manualBringNextPokemon"/);
...
assert.match(html, /const width = timelineZoomPercent\(\) < 100 \? 38 : 46/);
assert.match(html, /timeline-block\.switch-event img:first-child \{ margin-right: -6px/);
assert.doesNotMatch(html, /overview-mode \.timeline-block\.switch-event img:first-child,[\s\S]{0,100}display: none/);
assert.match(html, /Post-Charged switch/);
assert.match(html, /Free switch after your Charged Attack/);
assert.match(html, /chargedAttackActor: chargeEvent\.trainer/);
assert.match(html, /battleTurns\[side\] = currentTurn \+ result\.turnCost/);
assert.match(html, /postCharged: result\.postCharged/);
...
assert.doesNotMatch(html, /data-manual-technical-issue="dre"/, "The old DRE control must not be exposed in Manual Mode.");
assert.match(html, /data-manual-technical-issue="timing-anomaly"/, "Manual Mode must expose the Timing Anomaly control.");
assert.match(html, /function manualTimingAnomalyIssues\(\)[\s\S]{0,500}createTimingAnomalyIssues/, "Timing Anomaly eligibility must use the canonical pending Fast state.");
assert.match(html, /data-manual-timing-anomaly-target/, "Multiple pending Fast targets must be selectable explicitly.");
...
assert.match(html, /Reconstruct an observed timing irregularity by resolving a pending Fast Attack first\./);
assert.match(html, /function manualTechnicalEligibleEventIndexes/);
assert.match(html, /No pending Fast Attack is ready to reconstruct in the current state\./);
assert.match(html, /data-technical-target-label/);
assert.match(html, /Timing Anomaly target/);
