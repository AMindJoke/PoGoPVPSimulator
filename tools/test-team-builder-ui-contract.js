...
assert.match(html, /const rank = options\.showThreatSummary[^\n]+teamBuilderThreatGroups\.findIndex[^\n]+row \+ 1[\s\S]{0,500}class="team-coverage-rank">\$\{rank\}/, "Filtered coverage rows must preserve their original threat rank.");
...
assert.match(html, /data-result-tooltip="\$\{escapeHtml\(`\$\{category\} · Rating \$\{result\.score\}`\)\}"/, "Compact heatmap cells must disclose outcome strength and rating on hover or focus.");
