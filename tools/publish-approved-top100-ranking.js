...
const approvedPath = "reports/top100-fixed-weight-score-correction-20260909.json";
const approved = read(approvedPath);
...
for (const selected of approved.entries) {
  ...
  assert.equal(geometric(weighted.map(rating)), selected.overallScore);
  stats.forEach((s, i) => {
  ...
  entries.push(entry);
  const relevant = selectRelevantMatchups(cells, ranks, 5);
  const row = r => ({ id: r.opponentId, name: byId.get(r.opponentId).name, rank: r.opponentRank, score: r.score });
  details[entry.id] = { ratings: buildRankingRatings(entry, { complexity: { score: technicalScores.get(entry.id) } }), wins: relevant.wins.map(row), losses: relevant.losses.map(row) };
...
assert.deepEqual(entries.map(e => [e.id, e.rank, e.overallScore]), approved.entries.map(e => [e.id, e.rank, e.overallScore]));
